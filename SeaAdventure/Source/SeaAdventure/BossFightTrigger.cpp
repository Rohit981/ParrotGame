...
void ABossFightTrigger::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor != this && OtherComp->ComponentHasTag(FName("Player")))
	{
		...
		Cast<UBoxComponent>(BlockWall->GetComponentByClass(UBoxComponent::StaticClass()))->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
	}
}
...
void ABossFightTrigger::ResetBossBlockWall()
{
	Cast<UBoxComponent>(BlockWall->GetComponentByClass(UBoxComponent::StaticClass()))->SetCollisionEnabled(ECollisionEnabled::NoCollision);
}
